...
export default function App() {
  ...
  const askQuestion = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev.slice(0, -1), { role: "bot", text: "❌ Error connecting to backend." }]);
    }
    setLoading(false);
  };
      ...
      <div className="w-full max-w-2xl flex flex-col gap-5 relative z-10">
        ...
        <div className="text-center mb-2">
          ...
          <h1 className="text-5xl font-bold tracking-tight">
            <span className="bg-gradient-to-r from-white via-gray-200 to-gray-400 bg-clip-text text-transparent">Study</span>
            <span className="bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">Mind</span>
          </h1>
          <p className="text-gray-500 text-sm mt-2">Upload your notes · Ask anything · Get instant answers</p>
        </div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            {uploading ? (
              ...
              <div className="text-sm">
                ...
                <span className="text-gray-600 ml-2">· Click to replace</span>
              </div>
            ) : (
              <div className="text-sm text-gray-500 group-hover:text-gray-400 transition-colors">
                <span className="text-blue-400 font-medium">Click to upload</span> your PDF notes
              </div>
            )}
          </div>
        </div>
        ...
        >
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              style={msg.role === "bot" ? { background: "rgba(255,255,255,0.05)" } : {}}
              >
                {msg.loading ? (
        ...
        <p className="text-center text-gray-700 text-xs">
          Built by <span className="text-gray-500">Anshuman Dev</span> · RAG Pipeline · Gemini API · FastAPI · PostgreSQL
        </p>
